...
- 5차(2026-09-06): CLAUDE.md와 AGENTS.md는 항상 같은 내용 — 기존 폴더를 등록할 때 둘 중 하나만 있으면 나머지를 복제하고(토스트로 알림), 둘 다 없을 때만 생성 대화상자를 띄운다. 생성되는 두 파일도 같은 문서다(예전의 `@AGENTS.md` 스텁 폐지). `projects::docs_sync`가 등록된 모든 프로젝트 루트를 `notify`로 감시해 한쪽이 바뀌면 400ms 뒤 다른 쪽에 미러링한다(삭제하면 다시 복제, 빈 파일은 절대 원본이 되지 않음, 둘 다 다르면 최근 수정본 우선). 감시가 놓칠 경우를 대비해 앱 시작·프로젝트 선택·세션 시작·턴 종료 시에도 동기화를 돌린다. 앱 시작 시 이미 등록된 프로젝트에도 적용되므로 CLAUDE.md만 있던 프로젝트에는 AGENTS.md가 생긴다.
...
6. **WSL 지원을 코드에서 완전히 제거하고 Windows 네이티브만 지원한다.** (2026-09-08 사용자 결정) `backend/wsl.rs`·`paths.rs`·`managed/`(앱 전용 배포판)·`toolchain.rs`(shim)·온보딩의 실행 환경 선택·설정의 실행 환경 탭·WSL 통합 테스트를 모두 삭제했고, `ExecBackend`는 트레이트가 아니라 호스트 실행기 구조체 하나다. MSVC Build Tools는 일반 도구(`msvc`, vswhere 감지)로 편입해 Rust 계열 스택의 prerequisites에 넣었다. 체크포인트 git 명령은 `-c core.autocrlf=false`로 바이트를 그대로 보존한다(Git for Windows 기본값이 CRLF 변환). 이 PC 설정: 최신 Claude Code(2.1.263)는 Git for Windows 없이도 동작한다(없으면 Bash 도구 대신 PowerShell 도구 사용, 공식 문서 확인). 공식 설치 스크립트로 `%USERPROFILE%\.local\bin\claude.exe`에 설치하고 예전 winget 패키지(2.1.77, Git Bash 강제)는 제거했다. Codex는 `npm install -g @openai/codex`로 네이티브 설치. 설정은 `backend=native`, `claude_bin`은 절대 경로. WSL 백엔드 코드와 `*_wsl` 테스트는 그대로 유지한다. 앱의 git 패널·체크포인트는 여전히 `git.exe`가 필요하다.
7. **터미널 없이 전부 GUI로.** (2026-09-08 사용자 요청) 로그인은 `claude auth login --claudeai` / `codex login`을 숨은 PTY에서 돌리고 출력에서 로그인 URL·코드 입력 요청을 읽어 카드 안에 표시한다(`tools::login`, `LoginPanel`). Claude는 브라우저에 뜬 코드를 붙여넣어야 끝나고 Codex는 localhost 콜백으로 자동 완료. 도구 설치는 `tools_install`이 winget/설치 스크립트를 백그라운드로 돌리며 진행 줄을 표에 인라인으로 보여 준다. 온보딩의 터미널 토글과 "터미널에서 설치/로그인" 버튼은 없앴고, 설정의 실행 파일 경로에는 파일 선택 버튼을 붙였다. 터미널 패널은 선택 기능으로 남긴다.
