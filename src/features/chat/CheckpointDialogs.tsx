      ...
      <Dialog open={!!restoreTarget} onOpenChange={(o) => !o && !restoring && close()}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              "{restoreTarget?.label}" 복원 지점의 파일 상태로 프로젝트를 되돌립니다. 그 이후 바뀐 파일은 덮어쓰이거나 삭제됩니다. 되돌리기 전의 현재
              상태는 안전 복원 지점로 먼저 저장되므로 다시 복구할 수 있습니다.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
      ...
      <Dialog open={!!diffTarget} onOpenChange={(o) => !o && close()}>
        <DialogContent className="max-h-[85vh] sm:max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>복원 지점와 현재 작업 트리의 차이입니다.</DialogDescription>
          </DialogHeader>
          <div className="min-h-0 max-h-[65vh] overflow-auto">
            {diffLoading ? (
              ...
              <div className="py-6 text-center text-sm text-muted-foreground">복원 지점 이후 바뀐 파일이 없습니다.</div>
            )}
          </div>
        </DialogContent>
      </Dialog>
