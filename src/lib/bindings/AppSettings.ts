// This file was generated by [ts-rs](https://github.com/Aleph-Alpha/ts-rs). Do not edit this file manually.
import type { AutoGit } from "./AutoGit";
import type { Effort } from "./Effort";
import type { McpServerConfig } from "./McpServerConfig";
import type { PermissionPreset } from "./PermissionPreset";
import type { Provider } from "./Provider";

export type AppSettings = { default_provider: Provider, default_model_claude?: string | null, default_model_codex?: string | null, default_effort: Effort, default_permission: PermissionPreset, 
/**
 * Windows path where new projects are created by default.
 */
projects_root?: string | null, 
/**
 * Explicit binary overrides (host or backend path). Empty = auto-detect.
 */
claude_bin?: string | null, codex_bin?: string | null, git_bin?: string | null, 
/**
 * Commit author used for git commits made by the app (passed as `-c user.name/user.email`).
 */
git_user_name?: string | null, git_user_email?: string | null, theme: string, onboarding_done: boolean, 
/**
 * Snapshot the working tree before every agent turn (see `checkpoint`).
 */
checkpoints_enabled: boolean, 
/**
 * Windows notifications when a turn ends / approval is needed while the window is unfocused.
 */
notifications_enabled: boolean, 
/**
 * Check GitHub Releases for app updates on startup.
 */
auto_update_check: boolean, 
/**
 * MCP servers shared by both agents.
 */
mcp_servers: Array<McpServerConfig>, 
/**
 * Default for projects that do not override it: commit (and push) after every agent turn.
 */
auto_git: AutoGit, 
/**
 * Source checkout of this app for "새 빌드 적용" (None = auto-detect).
 */
dev_repo_path?: string | null, };
