...
const pbtn = [...aside.querySelectorAll("button")].find((b) => b.textContent.trim().startsWith("vibecode") && b.textContent.includes("tauri-react"));
if (pbtn) { vc.click(pbtn); await vc.sleep(1500); }
const toggle = document.querySelector('aside button[aria-label="UI 미리보기"]');
if (!toggle) throw new Error("UI 미리보기 button not found");
if (toggle.getAttribute("aria-pressed") !== "true") vc.click(toggle);
const cmdInput = await vc.waitFor(() => [...document.querySelectorAll("main input")].find((i) => (i.placeholder || "").includes("dev 서버 명령")), 15000, 300);
vc.setValue(cmdInput, window.__previewCmd || "python3 -m http.server 8124 --bind 0.0.0.0");
await vc.sleep(200);
const startBtn = [...document.querySelectorAll("main button")].find((b) => b.textContent.trim() === "시작");
vc.click(startBtn);
