...
vc.click(next);
await vc.waitFor(() => document.querySelector("h1")?.textContent === "준비 완료", 10000);
await vc.sleep(400);
const summary = document.body.innerText.slice(document.body.innerText.indexOf("준비 완료"), document.body.innerText.indexOf("이전\n시작하기"));
const start = [...document.querySelectorAll("button")].find((b) => b.textContent.trim().includes("시작하기"));
vc.click(start);
await vc.waitFor(() => !!document.querySelector('[data-drop-zone="projects"]'), 15000);
await vc.sleep(1500);
